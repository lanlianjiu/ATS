+ function ($) {
    var cfg = {
        server_: "http://localhost/ATS/backend/web/",
...
+
/**
 * data-region-plugin start 三级联动地址插件
 */
function ($) {
    $("[data-region-plugin]").each(function () {

        $this = $(this);

        var pluginUrl = $this.attr("data-region-plugin");
        var pluginId = $this.attr("id");
        var required = $this.attr("data-region-required");
        var valueType = $this.attr("data-region-valueType") || 'code';
        var dataType = $this.attr("data-region-dataType") || 'json';
        var nodata = $this.attr("data-region-nodata");
...
/**
 * bootstrap-select start 下拉框插件
 */
function ($) {

    $("select[data-select-plugins]").each(function () {

        $this = $(this);

        $htmlId = $("#" + $this.attr("id"));

        var URL = $this.attr("data-select-plugins");
        var idField = $this.attr("data-option-value");
        var keyField = $this.attr("data-option-label");
        var html = "<option value=''>" + '--请选择--' + "</option>";
        var ngModel = $this.attr("ng-model") || $this.attr("iov-ng-model");

        var inptHideId = $.utils.uuid();
        hideHtml = '<input type="hidden" ng-model="' + ngModel + '" ' + 'id="' + inptHideId + '" name=' + keyField + '_txt' + ' >';

        $this.before(hideHtml);

        $.post($.cfg.server_ + URL, {}, function (res) {

            var data = res.value;

            for (var i in data) {
                html += " <option value=" + data[i][idField] + ">" + data[i][keyField] + "</option>"
            };
        ...
        //监听改变事件
        $htmlId.on('changed.bs.select', function (e) {
            var checkValue = $htmlId.find("option:selected").attr('value');
            var hideInput = $("#" + inptHideId);
            hideInput.val(checkValue);
            $.angular.updateScope(hideInput);
        });

        //加载后时触发事件
        $htmlId.on('loaded.bs.select', function (e) {
...
+
/**
 * bootstrap-suggest-plugin start 建议搜索插件
 */
function ($) {

    $("div [data-suggestion-plugin]").each(function () {

        $this = $(this);
        var htmlId = $this.attr("data-suggestion-inputId");
        var htmlUrl = $this.attr("data-suggestion-plugin");
        var key = $this.attr("data-suggestion-key");
        var name = $this.attr("data-suggestion-name");
        var searchNames = $this.attr("data-suggestion-searchNames");
        var ngModel = $this.attr("iov-ng-model");

        var htmlTpl = '<div class="input-group">' +
            '<input data-suggestion-hide_input_' + htmlId + '="' + htmlId + '_input" class="hide" type="text" ng-model="' + ngModel + '_id"/>' +
            '<input type="text" ng-model="' + ngModel + '" class="input-text" id="' + htmlId + '" >' +
        ...
        $("#" + htmlId).bsSuggest({
            url: $.cfg.server_ + htmlUrl,
            ignorecase: true,
            showHeader: false,
            showBtn: true, //不显示下拉按钮
            delayUntilKeyup: true, //获取数据的方式为 firstByUrl 时，延迟到有输入/获取到焦点时才请求数据
            searchFields: [searchNames],
            idField: key,
            keyField: name,
            clearable: true,
            fnPreprocessKeyword: function (keyword) {
                //请求数据前，对输入关键字作预处理
                return $.trim($('#' + htmlId).val()) + keyword;
            }
            ...
            //选中数据
            var hideInput = $("input[data-suggestion-hide_input_" + htmlId + "='" + htmlId + "_input']");
            var showInput = $("#" + htmlId);

            hideInput.val(data[key]);
            $.angular.updateScope(hideInput);
            $.angular.updateScope(showInput);
...
/**
 * bootstrap-suggest-plugin end
 */
+
/**
 * WdatePicker start 日期插件
 */
function ($) {
    $("[data-WdatePicker-plugin]").each(function () {


    })
}(jQuery);
/**
 * WdatePicker end 日期插件
 */
+
/**
 * bootstrap-table-search start 表格初始化
 */
function ($) {
...
/**
 * bootstrap-table-search end 表格初始化
 */
+
/**
 * bootstrap-table-search start 表格搜索
 */
function ($) {
...
/**
 * bootstrap-table-search end 表格搜索
 */